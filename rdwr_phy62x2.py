...
class phyflasher:
	...
	def Connect(self, baud=DEF_RUN_BAUD):
		...
		GPIO.setup(RST_GPIO, GPIO.OUT, initial=GPIO.LOW)
		...
		# Reset and enter the ROM bootloader. Retry the complete reset
		# sequence if UXTDWU does not produce cmd>>: / fct>>:.
		for attempt in range(1, 101):
			print('')
			print('Bootloader attempt %d' % attempt)
			...
			while time.monotonic() < deadline:
				...
				if read == b'cmd>>:':
					print('Bootloader response: cmd>>:')
					break

				if read == b'fct>>:':
					print('Bootloader response: fct>>:')
					fct_mode = True
					break

				time.sleep(0.03)

			if read == b'cmd>>:' or read == b'fct>>:':
				break

			print('No bootloader response, retrying...')
			time.sleep(0.50)

		else:
			print('Chip Reset error! Response: %s' % read)
			print('Check GPIO25 -> RST_N, TX/RX, TM, and Chip Power!')
			self._port.close()
			exit(4)
